  ...
  <div class="c-coupon-activity">
    <div class="c-main-panel">
      <div class="c-coupon-panel" v-if="couponList.length">
        <div class="c-coupon-item" v-for="(item,index) in couponList" @click="selectCoupon(index)" v-cloak>
...
<script>
  import {Group, Cell, Blur, XButton, CheckIcon} from 'vux'

  export default {
    name: "select-coupon",
    components: {Group, Cell, Blur, XButton, CheckIcon},
    data() {
      return {
        args: {
          start: 0,
    ...
    methods: {
      ...
      listCouponCount() {
        this.$axios.post(`${this.$api.countUserCouponByDeviceId}`, $.param({
          deviceId: this.$route.params.deviceId,
        })).then((res) => {
          if (res.data.success) {
            this.coupon.couponMount = `可用优惠券${res.data.data}张`;
          } else {
            this.$vux.toast.text(res.data.retMsg, 'top');
          }
        }).catch((err) => {
      ...
      selectCoupon(index) {
        this.$store.state.coupon.couponValue = this.couponList[index].couponValue;
        this.$store.state.coupon.couponId = this.couponList[index].couponId;
        window.history.go(-1)
      }
    }
  }
